...
пайплайн подхватит без изменений в остальном коде.
  ...
  │ PairingStage     │ PairingContext                 │ List[SentimentPair]      │
...
from src.schemas.models import AggregationInput, PairingContext, PairingMetadata, SentimentPair
...
class ClusteringStage(ABC):
    ...
    def get_pairing_metadata(self) -> "PairingMetadata":
        from src.schemas.models import PairingMetadata
...
class PairingStage(ABC):
    """
    Стадия 5: построение NLI-пар из аспектов и скоренных кандидатов.
    PairingContext → List[SentimentPair]
    """
    @abstractmethod
...
class SentimentStage(ABC):
    """
    Стадия 6: NLI-сентимент для пар (sentence, aspect).
...
class AggregationStage(ABC):
    """
    Стадия 7: байесовская агрегация оценок по аспектам.
