...
# short ("чисто, но шумно") — we accept 2/2 here, then orphan-merge sweeps up anything
# that came out below 3 from causes other than discourse splits.
...
class RuleBasedClauseSegmenter:
    ...
    def _normalize(self, text: str) -> str:
        ...
        # Collapse ellipses to single dot.
    ...
    def _soft_split(self, chunk: _Chunk, source: str) -> list[_Chunk]:
        ...
        for marker, reason in markers_with_reason:
            for pos in self._find_marker_positions(text, marker):
                ...
                if not self._has_min_split_content(text[:cut_local]):
                    continue
                if not self._has_min_split_content(text[cut_local:]):
                    continue
    ...
    _SPLIT_DERIVED_BOUNDARIES: frozenset[str] = frozenset({"CONTRAST", "OPENER"})
